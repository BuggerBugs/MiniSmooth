...
# ============================================================
# CONFIGURE YOUR EXPERIMENT HERE (or pass as CLI args)
# ============================================================
...
def main():
    ...
    for seed in SEEDS:
        ...
        # Set env var so smoothquant.py saves JSON to our run dir
        sq_stats_path = str(run_dir / f"smoothquant_stats_{row_name}.json")
        env = os.environ.copy()
        env['SQ_STATS_PATH'] = sq_stats_path

        # Run ptq.py, streaming stdout directly to temp log file (avoids RAM buffering)
        cmd = ['python3', 'ptq.py', '--config', str(config_path)]
        ...
        # Parse accuracy from log file (delete later)
        top1, top5 = parse_accuracy_from_file(log_path)

        if top1 is None:
            print(f"  ⚠️  Could not parse accuracy from output for seed {seed}")
