...
def apply_smoothquant_to_prepared_model(model, calib_loader, alpha=0.5, device='cuda'):
    ...
    # ====================================================================
    # DUMP STATS TO FILE FOR VISUALIZATION (this version only shows theoretical and before. Full AFTER code is in verify_smoothquant.py)
    # ====================================================================
    import json
    dump = {}
    ...
    for name, module in conv_modules.items():
        if name not in act_stats:
            print(f"\n  Warning! Skip {name}: No stats collected")
            continue
        ...
        if hasattr(module, 'weight'):
            ...
            print(f"    Warning! Skip: No weight attribute")
            continue
        ...
        if act_max.shape[0] != weight_max.shape[0]:
            print(f"    Warning! Skip: dimension mismatch act={act_max.shape} weight={weight_max.shape}")
            continue
        
        # Calculate s, that will later be used to migrate quantization difficulty from activations to weights
        ...
        if s.max() > 10 or s.min() < 0.1:
            print(f"     WARNING: Extreme smoothing factors detected!")
        
        # Formula: Y = (X / s) @ (W * s)
        with torch.no_grad():
            weight.mul_(s.view(1, -1, 1, 1))
        
        print(f"    Succesfully modified weights: multiplied by s")
    ...
    for conv_name in scale_factors.keys():
        ...
        if quant_name is None:
            print(f"\n  Warning!  Could not find quantizer for {conv_name}")
            failed_count += 1
            continue
        ...
        try:
            ...
            print(f"    Successfullu stored inv_scale on {quant_name}")
            print(f"      inv_scale: mean={inv_scale.mean():.4f}, range=[{inv_scale.min():.4f}, {inv_scale.max():.4f}]")
            applied_count += 1
            
        except Exception as e:
            print(f"    Warning! Failed to apply scale: {e}")
            import traceback
            traceback.print_exc()
            failed_count += 1
            continue
    ...
    if failed_count > 0:
        print(f"  Warning!  Failed: {failed_count} quantizers")
...
def apply_smoothquant_scaling_hooks(model):
    ...
    for name, module in model.named_modules():
        if hasattr(module, '_smoothquant_inv_scale'):
            ...
            hooks.append(hook)
            print(f"  Successfully applied scaling hook to: {name}")
